...
ARCHIVO_EXCEL = "Carrera Laguna Seca.xlsx"
...
# --- GRÁFICO DE DONA CON POSICIONES Y LETRA MÁS GRANDE ---
            st.markdown("""
                <style>
            ...
            with col_centro:
                ...
                fig_dona.update_traces(
                    ...
                    # Aumentamos el tamaño de la fuente aquí (puedes probar con 16 o 18 si lo querés más grande)
                ...
                fig_dona.update_layout(
                    ...
                    margin=dict(l=60, r=60, t=30, b=30), # Ampliamos un poco los márgenes para que las etiquetas grandes no se corten
    ...
    if df_puntos_sim is not None and df_hoja1_sim is not None:
        ...
        for f_futura in range(cant_fechas_a_proyectar):
            ...
            # --- VALIDACIÓN DE POSICIONES REPETIDAS ---
            hay_duplicados_c1 = len(set(posiciones_f_c1.values())) < len(pilotos_torneo)
            hay_duplicados_c2 = len(set(posiciones_f_c2.values())) < len(pilotos_torneo)

            if hay_duplicados_c1 or hay_duplicados_c2:
                st.error("❌ **Error en la simulación:** No puedes repetir la misma posición para dos pilotos distintos en la misma carrera. Ajusta los puestos para que sean únicos (1°, 2°, 3° y 4°).")
                st.stop()
