...
def procesar_json_ac(archivo_json):
    data = json.load(archivo_json)
    
    # 1. Buscar la lista de resultados en ResultData o Sessions
    resultados_raw = data.get("ResultData", [])
    if not resultados_raw and "Sessions" in data:
        # Por si viene dentro del array de Sessions
        for session in data["Sessions"]:
            if session.get("Type") == "RACE" or session.get("Name") == "Race":
                resultados_raw = session.get("ResultData", session.get("RaceResult", []))
                break

    if not resultados_raw:
        return None

    # 2. Extraer los datos de cada piloto
    filas = []
    for pos, res in enumerate(resultados_raw, start=1):
        nombre = res.get("DriverName", f"Piloto {pos}")
        vueltas = res.get("NumLaps", 0)
        best_lap_ms = res.get("BestLap", 0)
        
        # Convertir BestLap de milisegundos a formato MM:SS.mmm
        if best_lap_ms > 0:
            mins = int(best_lap_ms // 60000)
            segs = (best_lap_ms % 60000) / 1000
            best_lap_str = f"{mins:02d}:{segs:06.3f}"
        else:
            best_lap_str = "N/A"
        ...
        filas.append({
            ...
            "Piloto": nombre,
            "Vueltas": vueltas,
            "Mejor Vuelta": best_lap_str,
            "BestLap_ms": best_lap_ms
        })
...
# ==========================================
st.sidebar.markdown("## 🏎️ Torneo TC")
...
# --- BLOQUE DE CARGA DE JSON ---
st.sidebar.markdown("---")
st.sidebar.subheader("📤 Cargar Resultados (.json)")
archivo_json = st.sidebar.file_uploader("Sube el JSON de la carrera", type=["json"])

if archivo_json is not None:
    df_carrera = procesar_json_ac(archivo_json)
    if df_carrera is not None:
        st.sidebar.success("¡JSON cargado correctamente!")
        st.sidebar.dataframe(df_carrera[["Posición", "Piloto", "Vueltas", "Mejor Vuelta"]], use_container_width=True)
    else:
        st.sidebar.error("No se encontraron datos de carrera en el archivo.")

st.sidebar.markdown("---")

opcion = st.session_state['pagina_activa']
# CÓDIGO NUEVO (Lee dinámicamente los pilotos desde el Excel):
try:
    # Lee los nombres directamente de la hoja del Excel
    df_excel = pd.read_excel(ARCHIVO_EXCEL, sheet_name=0)  # Lee la primera hoja del Excel
    if "Piloto" in df_excel.columns:
        pilotos = df_excel["Piloto"].dropna().unique().tolist()
    else:
        # Si la primera columna no se llama Piloto, toma los valores de la primera columna
        pilotos = df_excel.iloc[:, 0].dropna().unique().tolist()
except Exception:
    pilotos = []

df_resumen = pd.DataFrame({
    "Piloto": pilotos,
    "Puntos": [0] * len(pilotos),
    "Victorias": [0] * len(pilotos),
    "Poles": [0] * len(pilotos),
    "Lastre Actual (kg)": [0] * len(pilotos)
})
# ==========================================
...
if opcion == "Resumen":
    ...
    st.write(f"Sincronizado con tu archivo local {ARCHIVO_EXCEL}")
